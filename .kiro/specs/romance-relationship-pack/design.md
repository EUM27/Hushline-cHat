...
대학 졸업반 무렵, {{유저}}가 우연히 들어간 오래된 셰어하우스. 먼저 살고 있던 세 사람과
...
|----|------|-------|-----------|----------------|
| `seo-yujin` | 서유진 | 밝고 직진형. 먼저 다가오지만 속은 외로움 | 高 외향/개방, 低 신경성 | +2 |
| `han-doyun` | 한도윤 | 무뚝뚝한 완벽주의자. 거리 두지만 챙김 | 高 성실, 低 외향 | 0 |
| `mer-ari` | 메아리 | 오랜 소꿉친구. 편하지만 변화를 두려워함 | 高 우호/신경성 | +3 |
...
mer-ari   ──(서유진을 부러워함: envy 5)──> seo-yujin
seo-yujin ──(아리를 편하게 여김: fondness 6)──> mer-ari
mer-ari   ──(유저에게 오랜 마음: hidden_affection 7)──> (유저는 relationshipToUser로 표현)
```

유저를 둘러싼 삼각: 서유진(직진) vs 메아리(오랜 마음) + 한도윤(천천히 여는 쪽). Director가
관계를 어떻게 흔드는지가 핵심 검증 포인트.
...
### characters/*.json (3개)
각 캐릭터는 yoon-haeon.json 패턴을 따른다:
- `handout.secret`: 미스터리가 아니라 "감정적 비밀"(예: 메아리는 오랜 마음을 숨김). NPC는 직접
  밝히지 않고 behaviorRules로 표현.
- `surfacePersonality`/`fear`/`behaviorRules`/`desire`/`objective`/`initialRelationshipToUser` 채움.
- `relationships`: 위 관계 구도 반영.
- systemPrompt/behaviorRules에 "유저 행동·대사·감정 대리 서술 금지" 포함(기존 경계와 일관).
