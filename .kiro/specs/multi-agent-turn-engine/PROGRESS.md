...
## 2026-05-30 (세션 9 — 캐릭터 카드 import: PNG/JSON)

> 새 spec: `.kiro/specs/character-card-import/`.
> 카드 파일(JSON 또는 PNG)을 업로드해 `CharacterDefinition`으로 변환·미리보기하는 import
> 파이프라인. 세션 8의 카드 변환 경로(`cardToCharacterDefinition`) 위에 ⓐ PNG 청크 파서 +
> ⓑ 통합 import + ⓒ API 라우트 + ⓓ 클라 업로드 UI를 얹음. 외부 의존성 없음(node:zlib만).

**구현 (전부 통과):**
- [x] `server/engine-v2/png-card.ts` — `extractCardFromPng`: PNG 시그니처 검증 + tEXt/zTXt 청크 순회 + `ccv3` 우선·`chara` 폴백 + base64 디코드 (zTXt는 `inflateSync`)
- [x] `server/engine-v2/card-importer.ts` — `importCardJson`/`importCardPng` (characterCardSchema 검증 → cardToCharacterDefinition, 안전 오류 결과)
- [x] `server/app-v2/card-routes.ts` — `POST /api/v2/character-card/import` (kind png/json 감지, 크기 상한 8MB, 200/400/413), `cardImportBodySchema`
- [x] `client/api-v2.ts` — `importCharacterCard(file)` (readAsDataURL → base64 → POST, 확장자/MIME로 kind 결정)
- [x] `client/components/setup/CharacterCardImport.tsx` — 파일 선택 + 미리보기(이름/MBTI/표면성격/자율성/호감/관계/비밀) + 오류 표시, AdvisorSetupPanel에 편입 + setup.css 스타일
- [x] 테스트 14개: png-card 단위(ccv3/chara/zTXt/없음/시그니처 불량) + import 단위(JSON/PNG 동치, 스키마/비JSON 거부) + 라우트(PNG/JSON 200, 잘못된 본문 400, 카드 없음 PNG 400) — 합성 PNG(시그니처+IHDR+tEXt/zTXt+IEND, CRC32 직접 계산)

**효과:** ②(카드 import) 완결. JSON이든 PNG든 동일 `CharacterDefinition`으로 변환됨. 미리보기까지가
스코프이고, 세션/팩 주입(`replaceCharacterSlot`)은 후속. 외부 패키지 0.

**검증:**
- `corepack pnpm -r run check` → 통과 (shared/client/server)
- 서버 테스트 → 184 pass / 0 fail (170 → +14)
- 클라이언트 빌드 → 통과 (1738 modules)

### 다음 세션 후보
- ③ 페르소나 시스템 (유저 페르소나 정의·주입)
- 카드 import → 세션 슬롯 주입 연결 (`replaceCharacterSlot`)
- 멀티턴 플레이스루 통합 테스트 / CI / 메신저 멀티 대화방 2단계

---

## 2026-05-30 (계획 정리 — Portable Pack 설계 + 다음 작업 = 페르소나)

> 구현 없음. 설계/계획 문서만 정리한 세션.

### 정리한 설계 문서
- `portable-pack-plan.md` (rev2) — **단일 파일(JSON) 시나리오 팩** import/export/세션화 설계안.
  - 현재 팩은 이미 디렉터리 단위로 디스크 로드됨. 공백은 (1)단일 파일 포맷 (2)업로드 실행 경로
    (3)세션 자립성(`reconstructPack` 빈 껍데기) (4)기본 템플릿.
  - 핵심 결정(리뷰 반영): ① `embeddedPack`을 SessionState에 직접 X → `packSource` + 서버측
    `portable_packs` 스냅샷 참조(클라 누출 차단), ② PortablePackV1(외부 계약) ↔ ScenarioPack(런타임)
    분리 + `NormalizedPackDraft` 중간 수렴점으로 검증 일원화, ③ 업로드 = validate→import→create 3단계.
  - contentHash 필수(`id@sha256`), ClientPackMeta만 노출, round-trip은 semantic equivalence,
    v1 asset은 참조만(binary 금지), 템플릿 3종(chat/mystery/scene-device). Invariants 10개 명시.
  - **상태: 설계만. 정식 spec 미착수.**

### 다음 작업 = ③ 페르소나 시스템 (확정)
- 계획 메모: `persona-system-plan.md`.
- 진단: 엔진·서버 절반은 이미 구현·연결됨(`PersonaBrief`/`buildPersonaBrief`/파이프라인 주입/
  캐릭터 프롬프트 `formatUserPersonaForCharacter`/서버 라우트 매핑). **공백은 입력단 2곳** —
  (1) `createSessionBodySchema.persona`가 name만 받음(role/description/relationshipTags 결손),
  (2) 클라 `PersonaSetupPanel`이 이름만 입력받고 `createSessionV2`가 `{name}`만 전송.
- 범위: 순수 "입력 경로 끝까지 연결"(스키마 필드 + 클라 UI/배선 + 테스트). 토큰 적고 가치 큼.
- 진입점 파일 목록은 `persona-system-plan.md` §5 참조.

### 우선순위 메모
1. **③ 페르소나 시스템** (다음 작업, 입력단 연결)
2. Portable Pack (단일 파일 팩) — `portable-pack-plan.md` rev2 기준 spec 착수
3. 카드 import → 세션 슬롯 주입(`replaceCharacterSlot`) / 멀티턴 플레이스루 테스트 / CI / 메신저 멀티 대화방 2단계
